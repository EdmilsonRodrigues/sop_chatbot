...
async def run_migrations():
    async for migration in get_migrations():
        ...
        db.migrations.insert_one({"name": migration_name, "run_at": datetime.now()})
    print("All migrations ran successfully")


async def get_migrations() -> AsyncGenerator[Callable, None]:
    all_migrations = [
    ...
    for migration in all_migrations:
        ...
        if version > MAIN_VERSION:
            ...
            continue
        elif version == MAIN_VERSION and subversion == SUB_VERSION and patch > PATCH:
            continue
        migration = import_module(migration.replace("/", ".").replace(".py", ""))
