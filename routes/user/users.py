...
@router.put("/password", response_model=UserResponse, response_class=ORJSONResponse)
async def update_my_password(
    session: Annotated[User, Depends(session_dependency)],
    old_password: Annotated[str, Body(description="The old password", embed=True)],
    new_password: Annotated[str, Body(description="The new password", embed=True)],
):
    ...
    await db[session.table_name()].update_one({"_id": ObjectId(session.id)}, {"$set": {"password": Auth.encrypt_password(new_password)}})
    return session.json()
